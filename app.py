...
def buyer_dashboard():
    try:
        ...
        flash('An error occurred while loading the buyer dashboard', 'error')
        session.rollback()
    finally:
        session.close()        
...
def owner_page():
    ...
    try:
        ...
        flash('An error occurred on the owner page', 'error')
        session.rollback()
    finally:
        session.close()
    return render_template('owner_page.html', name=owner.first_name)
...
def landing_page():
    try:
        ...
        flash('An error occurred while loading the landing page', 'error')
        session.rollback()
    finally:
        session.close()
...
def order_page(dress_id):
    try:
        ...
        if not user.is_authenticated:
            return redirect(url_for('login'))
        # Eagerly load the buyer relationship
        user = session.query(User).options(joinedload(User.buyer)).filter(User.id == user.id).first()
        if request.method == "POST":
            if user.role == 'buyer':
                buyer = user.buyer
                if buyer:
